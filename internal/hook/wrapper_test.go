...
func TestWrapperXargsAppendedArgsDenyBeatsTrailingAllow(t *testing.T) {
	// The allow path probes the appended form, so deny/ask must too. Otherwise a
	// trailing-tolerant allow could approve `xargs rm /tmp/safe` while its runtime
	// form `rm /tmp/safe <stdin>` slips a deny that only matches with trailing
	// args. Deny still wins.
	allow := patterns("Bash(rm /tmp/safe*)")
	deny := patterns("Bash(rm /tmp/safe *)") // space before * → matches only WITH trailing args
	result := Process(bash(`printf '/tmp/other\n' | xargs rm /tmp/safe`), allow, nil, deny, nil, nopLog())
	assert.Equal(t, ResultDenyRule, result.Kind)

	// Same for an ask rule that only matches the appended form.
	ask := patterns("Bash(rm /tmp/safe *)")
	asked := Process(bash(`printf '/tmp/other\n' | xargs rm /tmp/safe`), allow, ask, nil, nil, nopLog())
	assert.Equal(t, ResultAsk, asked.Kind)
}
...
func TestWrapperXargsFindAppendsActionForcesAsk(t *testing.T) {
	// xargs appends stdin tokens to the find *expression*, so a mutating action
	// can be injected at run time and never appears in the args we classify.
	// `printf '-delete' | xargs find . -exec grep X {} \;` actually runs
	// `find . -exec grep X {} \; -delete`. Approving it from the -exec grep
	// payload alone would delete files, so an append-mode find forces a prompt.
	allow := patterns("Bash(grep:*)")
	asked := Process(bash(`printf -- '-delete\n' | xargs find . -exec grep X {} \;`), allow, nil, nil, nil, nopLog())
	assert.Equal(t, ResultAsk, asked.Kind, "append-mode xargs wrapping find must not auto-approve")

	// Replace mode substitutes at the placeholder instead of appending, so the
	// find expression is fully visible and a safe -exec payload still classifies
	// normally.
	ok := Process(bash(`printf 'dir\n' | xargs -I@ find @ -exec grep X {} \;`), allow, nil, nil, nil, nopLog())
	assert.Equalf(t, ResultAllowed, ok.Kind, "reason: %s", ok.Reason)
}
